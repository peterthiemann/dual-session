...
rules for \AgdaName{DualStopAtMu}{dualS}{dualS} and
...
Session-type duality goes back to the standard binary-session account and its
subtyping variants~\cite{gay-hole-subtyping}.  The present work is not a new
session calculus, but a mechanized design note about where the duality operator
should be defined in the presence of recursive types.

\section{Conclusion}
\label{sec:conclusion}

Stopping duality at $\mu$ turns recursive unfolding into a responsibility of
observation, rather than a burden on syntactic duality.  Equivalence remains
an auxiliary tool for payloads and for comparing the results extensionally;
the primary comparison is between coinductive duality and duality induced by
...
The main files in the current development are:
\begin{itemize}
\item \AgdaFile{Types.IND1}{Types/IND1.agda}: indexed syntax with renamings
  and substitutions;
\item \AgdaFile{Types.COI}{Types/COI.agda}: coinductive ground-truth model;
\item \AgdaFile{DualStopAtMu}{DualStopAtMu.agda}: stopped duality and ground
  comparison;
\item \AgdaFile{Conversion}{Conversion.agda}: syntactic conversion judgments
  and conversion soundness;
\item \AgdaFile{StopDualSoundness}{StopDualSoundness.agda}: soundness and
  completeness of stopped duality with respect to coinductive duality;
\item \AgdaFile{ConversionStopDualSoundness}{conversion bridge}: conversion,
  stopped duality, and equivalence; and
\item \AgdaFile{Examples}{Examples.agda}: extracted examples and
  counterexamples.
\end{itemize}
